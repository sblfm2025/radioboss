/* ==========================================================================
   LOGIKA INTERAKTIF JAVASCRIPT - RADIOBOSS FRESH RESET MANAGER
   Mengelola: Navigasi Tab, Fetch API, Live Update, Animasi Progress, & Toasts
   ========================================================================== */
...
async function triggerScan() {
    ...
    // Sediakan simulasi progress bar interaktif stasiun radio
    const progressSection = document.getElementById("scan-progress-section");
    const progressBar = document.getElementById("scan-progress-bar");
    const progressText = document.getElementById("progress-scanned-files");
    const elapsedText = document.getElementById("progress-elapsed");
    
    progressSection.classList.remove("hidden");
    progressBar.style.width = "0%";
    progressText.textContent = "Menginisialisasi pemindaian filesystem...";
    
    let seconds = 0;
    const timer = setInterval(() => {
        seconds++;
        elapsedText.textContent = `Waktu: ${seconds}s`;
    }, 1000);
    
    // Buat efek animasi bar dinamis
    let progressVal = 0;
    const progressAnimator = setInterval(() => {
        if (progressVal < 90) {
            progressVal += 15;
            progressBar.style.width = `${progressVal}%`;
            progressText.textContent = `Memindai folder audio... (${progressVal}%)`;
        }
    }, 200);
    ...
    try {
        ...
        if (res.status === "SUCCESS") {
            ...
            progressText.textContent = `Scan sukses! Menemukan ${res.data.audio_count} file audio dan ${res.data.playlist_count} playlist.`;
            showToast("Pemindaian, klasifikasi, dan audit otomatis sukses selesai!");
            
            // Muat ulang stats dashboard
            loadDashboardStats();
        } else {
            progressText.textContent = `Gagal: ${res.message}`;
            showToast(res.message, "error");
        }
    } catch (err) {
        clearInterval(timer);
        clearInterval(progressAnimator);
        progressText.textContent = "Terjadi kegagalan koneksi server API.";
...
async function triggerCopyFiles() {
    showToast("Memulai proses penyalinan aset. Silakan tunggu...", "info");
    ...
    try {
        ...
        if (res.status === "SUCCESS") {
            showToast(`Penyalinan selesai! Berhasil menyalin ${res.total_copied} file. Konflik nama: ${res.conflicts}`);
            loadDashboardStats();
        } else {
            showToast(res.message, "error");
        }
...
async function triggerGenerateReports() {
    try {
        ...
        if (res.status === "SUCCESS") {
            showToast("Seluruh laporan CSV/MD berhasil diekspor ke output/reports/ & folder steril!");
        } else {
            showToast(res.message, "error");
        }
    } catch (err) {
