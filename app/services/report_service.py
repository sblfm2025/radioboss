...
class ReportService:
    """
    Mengelola kompilasi dan ekspor seluruh berkas laporan CSV dan dokumen Markdown
    ke folder output dan folder steril laporan stasiun radio.
    """
    @classmethod
    def generate_all_reports(cls, progress_callback=None):
        """
        Mengambil data dari SQLite dan mengekspor seluruh laporan CSV dan Markdown
        yang diwajibkan stasiun radio.
        """
        if progress_callback:
            progress_callback(10, "Menginisialisasi kompilasi data laporan...")
            
        conn = get_db_connection()
        cursor = conn.cursor()
        
        fresh_root = load_fresh_root()
        output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'output', 'reports')
        ...
        try:
            if progress_callback:
                progress_callback(20, "Mengekspor drive_scan_summary.csv...")
        ...
        try:
            if progress_callback:
                progress_callback(30, "Mengekspor playlist_audit.csv...")
        ...
        try:
            if progress_callback:
                progress_callback(40, "Mengekspor ramadhan_adzan_report.csv...")
            cursor.execute("""
        ...
        try:
            if progress_callback:
                progress_callback(50, "Mengekspor dirty_metadata_report.csv...")
            cursor.execute("""
                SELECT original_path, file_name, detected_type, risk_level, status, notes 
                FROM file_index 
        ...
        try:
            if progress_callback:
                progress_callback(60, "Mengekspor old_source_path_usage.csv...")
            cursor.execute("""
        ...
        try:
            if progress_callback:
                progress_callback(70, "Mengekspor missing_paths_report.csv...")
            cursor.execute("""
        ...
        try:
            if progress_callback:
                progress_callback(80, "Mengekspor before_after_reset_summary.md...")
            # Kueri statistik
        ...
        try:
            if progress_callback:
                progress_callback(90, "Mengekspor final_setup_checklist.md...")
            # Buat folder steril target laporan stasiun radio
            fresh_report_dir = os.path.join(fresh_root, "09_LAPORAN_RESET")
            os.makedirs(fresh_report_dir, exist_ok=True)
            
            md_checklist_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'output', 'final_setup_checklist.md')
            ...
            # Tulis salinan ke folder steril
        ...
        # Simpan status ekspor laporan ke JSON manifest
        app_output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'output')
        try:
        ...
        logging.info("Pembuatan seluruh berkas laporan stasiun radio selesai.")
        if progress_callback:
            progress_callback(100, "Ekspor Laporan Sukses! Seluruh CSV dan dokumen Markdown siap diunduh.")
